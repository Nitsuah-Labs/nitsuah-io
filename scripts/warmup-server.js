...
const MIN_HTML_LENGTH = 10_000; // Minimum bytes indicating fully compiled page
...
async function warmup() {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (html.length > MIN_HTML_LENGTH && html.includes("<body")) {
        console.log(
          `✅ Dev server warmed up after ${attempt} attempts (${html.length} bytes)`
        );
        process.exit(0);
      } else {
        console.log(
          `⏳ Attempt ${attempt}/${maxAttempts}: Got ${html.length} bytes, waiting for compilation...`
        );
      }
    } catch (error) {
      if (attempt === 1) {
        console.log(
          `⏳ Attempt ${attempt}/${maxAttempts}: Server not ready, waiting...`
        );
      }
    }
