...
- add a "add a new blog" button/form to the blogs/ page
- the new label colors look good, but also look through our labels and be sure they are accurate to match all of our projects/crypto cards. maybe we missed some (ie: buildspace label should be on a few more, as should coinbase for the coinbase/coinbassador ones). dont over do it, just go through and try and make them match and be a bit more exclusive (ie: consolidating labels if they only have 1 match but two are similar enough to be combined into a new keyword/etc.)
